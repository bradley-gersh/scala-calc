  ...
  private def parseTerm(parseState: NestedParseState): Either[String, NestedParseState] = {
    if (parseState.tree.isEmpty) {
      parseFactor(parseState) match {
        case Left(error) => Left(error)
        case Right(parseStateAfterLeft) => parseTermRight(parseStateAfterLeft)
      }
    } else parseTermRight(parseState)
  }

  private def parseTermRight(parseState: NestedParseState): Either[String, NestedParseState] = {
  ...
  private def parseFactor(parseState: NestedParseState): Either[String, NestedParseState] = {
    ...
    if (leftRootIn.isEmpty) {
      parseSign(SimpleParseState(tokens, parens)) match {
        case Left(error) => Left(error)
        case Right(parseStateAfterLeft) => parseFactorRight(parseStateAfterLeft)
      }
    } else parseFactorRight(parseState)
  }

  private def parseFactorRight(parseState: NestedParseState): Either[String, NestedParseState] = {
    val NestedParseState(tokensAfterLeft, leftRoot, newParensLeft) = parseState
