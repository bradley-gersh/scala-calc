...
object Parser {
   ...
   * @return        Either an error or the parse tree determined by the tokens
   */
  def parse(tokens: List[Token]): Either[CalculatorException, ParseNode] = parseRoot(tokens)
   ...
   * @return                 Either an error or the parse tree determined by the tokens
   */
  private def parseRoot(tokens: List[Token]): Either[CalculatorException, ParseNode] =
    if (tokens.nonEmpty) {
      parseExpression(NestedParseState(tokens, EmptyNode, List[Token]())) match {
        ...
        case Right(NestedParseState(excessTokens, _, _)) if excessTokens.nonEmpty =>
          if (excessTokens.contains(RPAREN)) Left(UnmatchedParenthesesException("unmatched `)`"))
          else Left(UnparsedTokensException(s"unparsed tokens: ${excessTokens.foldLeft(mutable.StringBuilder())((acc, token) => acc.append(token.string))}"))

        case Right(NestedParseState(_, _, excessParens)) if excessParens.nonEmpty =>
          Left(UnmatchedParenthesesException(s"unmatched `(`: depth ${excessParens.length}"))
   ...
   * @return                 Either an error or a parse state containing:
   ...
   */
  @tailrec
  private def parseExpression(parseState: NestedParseState): Either[CalculatorException, NestedParseState] =
    ...
    tokens match {
      ...
      case RPAREN :: rest =>
        if (parens.isEmpty) Left(UnmatchedParenthesesException("unmatched `)`"))
   ...
   * (Note: Helper method means that calls are not tail-recursive.)
   *
   * @param  parseState      wrapper for current parse state, containing:
   *                           1. tokens:     list of tokens remaining to be parsed in this expression
   ...
   *                           3. parens:     a list of unclosed parentheses after parsing this expression
   */
  private def parseTerm(parseState: NestedParseState): Either[CalculatorException, NestedParseState] = {
    if (parseState.tree.isEmpty) {
      parseFactor(parseState) match {
        case Left(error) => Left(error)
        case Right(parseStateAfterLeft) => parseTermBranch(parseStateAfterLeft)
      }
    } else parseTermBranch(parseState)
  }

  /**
   * Parses one branch of a term
   *
   * @param  parseState      wrapper for current parse state, containing:
   *                           1. tokens:     list of tokens remaining to be parsed in this expression
   *                           2. leftRootIn: node at the root of the parse tree of the first argument to this function
   ...
   *                           3. parens:     a list of unclosed parentheses after parsing this expression
   */
  private def parseTermBranch(parseState: NestedParseState): Either[CalculatorException, NestedParseState] = {
    val NestedParseState(tokensAfterLeft, leftRoot, newParensLeft) = parseState
   ...
   *                           3. parens:     a list of unclosed parentheses after parsing this expression
   */
  private def parseFactor(parseState: NestedParseState): Either[CalculatorException, NestedParseState] = {
    val NestedParseState(tokens, leftRootIn, parens) = parseState

    if (leftRootIn.isEmpty) {
      parseSign(SimpleParseState(tokens, parens)) match {
        case Left(error) => Left(error)
        case Right(parseStateAfterLeft) => parseFactorBranch(parseStateAfterLeft)
      }
    } else parseFactorBranch(parseState)
  }

  /**
   * Parses one branch of a factor
   *
   * @param  parseState      wrapper for current parse state, containing:
   *                           1. tokens:     list of tokens remaining to be parsed in this expression
   *                           2. leftRootIn: node at the root of the parse tree of the first argument to this function
   ...
   *                           3. parens:     a list of unclosed parentheses after parsing this expression
   */
  private def parseFactorBranch(parseState: NestedParseState): Either[CalculatorException, NestedParseState] = {
    val NestedParseState(tokensAfterLeft, leftRoot, newParensLeft) = parseState
   ...
   *                           3. parens:     a list of unclosed parentheses after parsing this expression
   */
  private def parseSign(parseState: SimpleParseState): Either[CalculatorException, NestedParseState] = {
    val SimpleParseState(tokens, parens) = parseState

    tokens match {
      case Nil => Left(EmptyNodeException("expression terminated where a value was expected"))
   ...
   *                           3. parens:     a list of unclosed parentheses after parsing this expression
   */
  private def parseNumber(parseState: SimpleParseState): Either[CalculatorException, NestedParseState] = {
    val SimpleParseState(tokens, parens) = parseState
    ...
    tokens match {
      ...
      case NUMBER(string) :: rest =>
        if (string.toDouble.isInfinite) Left(InfiniteValueException("infinite value obtained"))
      ...
      case _ => Left(EmptyNodeException(s"found `${tokens.head.string}` where a value was expected"))
    }
  }
}
